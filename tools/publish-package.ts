...
if (!packagePath || !expectedVersion) {
  console.error('Usage: node tools/publish-package.ts <package.tgz> <expected-version>');
  process.exit(1);
}

const resolvedPackagePath = resolve(packagePath);
const localIntegrity = `sha512-${createHash('sha512')
  .update(await readFile(resolvedPackagePath))
  .digest('base64')}`;
...
if (registryIntegrity) {
  ...
  console.log(`vampire@${expectedVersion} already matches the verified package.`);
} else {
...
async function publishedIntegrity(version: string): Promise<string | undefined> {
  try {
    ...
    const details = `${commandError.stdout || ''}\n${commandError.stderr || ''}`;

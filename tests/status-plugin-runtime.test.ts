...
import { StatusPluginRuntime } from '../runtime/status-plugins.ts';
import type { StatusPluginCommandOptions } from '../src/lib/server/status-plugin-command.ts';
import {
	STATUS_PLUGIN_STATE_VERSION,
	type StatusPluginStore
} from '../src/lib/server/status-plugin-store.ts';
import {
...
test('starts one execution, prevents overlap, and broadcasts output without command source', async () => {
	...
	const runtime = new StatusPluginRuntime(
		(update) => snapshots.push(update),
		{
			readStore: async () => ({
...
test('runs CPU and RAM through the same ordered runtime contract', async () => {
	const runtime = new StatusPluginRuntime(() => undefined, {
		readStore: async () => ({
			version: STATUS_PLUGIN_STATE_VERSION,
			plugins: [
				{ id: 'ram', name: 'Memory', enabled: true, intervalMs: 2_000, source: { type: 'command', command: STATUS_PLUGIN_MEMORY_COMMAND } },
				{ id: 'cpu', name: 'Processor', enabled: true, intervalMs: 2_000, source: { type: 'command', command: STATUS_PLUGIN_CPU_COMMAND } }
			]
		}),
		runCommand: async (command) => ({
			stdout: command === STATUS_PLUGIN_CPU_COMMAND
	...
	await runtime.start();
	await waitFor(() => runtime.snapshots().length === 2
		&& runtime.snapshots().every((snapshot) => snapshot.state === 'ready'));
	assert.deepEqual(runtime.snapshots().map(({ id, name, text }) => ({ id, name, text })), [
...
test('runs every enabled plugin once when saved configuration changes', async () => {
	...
	assert.deepEqual(runtime.snapshots().map((snapshot) => snapshot.id), ['two', 'one']);
...
test('reuses a recent server result when every browser disconnects and reconnects', async () => {
	...
	await runtime.start();
	assert.deepEqual(runtime.snapshots().map((snapshot) => snapshot.text), ['12%']);
	await new Promise((resolve) => setTimeout(resolve, 25));
	assert.equal(executions, 1);
	runtime.stop();
});

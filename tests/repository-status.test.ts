...
import { closeRepositoryStatusObservers, observeRepositoryStatus } from '../runtime/repository-status.ts';
...
test('pushes Git change counts and releases its watcher with the socket', async (t) => {
	...
	await writeFile(join(stateDirectory, 'sessions.json'), JSON.stringify({
		version: 1,
		sessions: [{ id: sessionId, tmuxSession: 'vampire-e272a1ce', cwd: workspace }]
	}));
	...
	await waitFor(() => socket.messages.filter((message) => message.type === 'repository-status').at(-1)?.changeCount === 0);

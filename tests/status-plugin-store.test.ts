...
test('starts with CPU and RAM presets and persists an explicitly empty bar', async (t) => {
	...
	assert.deepEqual((await readStatusPluginStore(file)).plugins.map((plugin) => plugin.name), ['CPU', 'RAM']);
...
test('converts legacy native CPU and RAM entries into visible commands', async (t) => {
	const directory = await mkdtemp(join(tmpdir(), 'vampire-status-plugin-legacy-'));
	t.after(() => rm(directory, { recursive: true, force: true }));
	const file = join(directory, 'status-plugins.json');
	...
	const stored = await readStatusPluginStore(file);
	assert.deepEqual(stored.plugins.map((plugin) => plugin.source.command), [
		STATUS_PLUGIN_CPU_COMMAND,
		STATUS_PLUGIN_MEMORY_COMMAND
	]);
});
...
test('refuses malformed status plugin state instead of overwriting it', async (t) => {
	...
	await writeFile(file, JSON.stringify({
		version: STATUS_PLUGIN_STATE_VERSION,
		plugins: [{ id: 'unsafe', name: 'Unsafe', enabled: true, intervalMs: 1, source: { type: 'command', command: 'date' } }]
	}));

...
import * as tmux from '../src/lib/server/tmux.ts';
...
test('follows a single foreground child without parsing command arguments', () => {
	...
	const [session] = tmux.parseTmuxSessions('workspace\t1\t0\t0\t@0\truntime\t1\t2\t%0\truntime\t10\t', processes);

	assert.deepEqual(session.foregroundProcess, { kind: 'command', label: 'agent' });
});

test('groups tmux windows while keeping background activity out of the main session timestamp', () => {
	const [session] = tmux.parseTmuxSessions([
		'workspace\t1\t2\t0\t@0\tprimary\t0\t3\t%0\tcodex\t0\t',
		'workspace\t1\t2\t1\t@1\tserver\t1\t5\t%1\tnode\t0\t'
	].join('\n'));

	assert.equal(session.name, 'workspace');
	assert.equal(session.lastOutputAt, 3_000);
	assert.deepEqual(session.foregroundProcess, { kind: 'command', label: 'codex' });
	assert.deepEqual(session.terminals, [
		{
			id: '@0',
			index: 0,
			name: 'primary',
...
test('describes managed background commands and their exit status', () => {
	const sessions = tmux.parseTmuxSessions([
		['background-running', '1', '0', '1', '@1', 'pnpm dev', '0', '5', '%1', 'node', '0', '', 'pnpm dev', '1712345678000', '0', ''].join('\t'),
		['background-exited', '1', '0', '1', '@2', 'tests', '0', '6', '%2', 'zsh', '0', '', 'pnpm test', '1712345680000', '1', '7'].join('\t')
	].join('\n'));

	assert.deepEqual(sessions[0].terminals[0], {
		id: '@1',
		index: 1,
		name: 'pnpm dev',
		active: false,
	...
	assert.deepEqual(sessions[1].terminals[0], {
		id: '@2',
		index: 1,
		name: 'tests',
		active: false,
...
test('parses the lightweight tmux output activity snapshot', () => {
	assert.deepEqual(tmux.parseTmuxSessionActivity([
		...
		'vampire-one\t1\t1712345682',
		'vampire-two\t3\t1712345680'
	].join('\n')), [

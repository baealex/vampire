...
const namedColorDeclaration =
  /(?:color|background(?:-color)?|border(?:-[\w-]+)?|fill|stroke)\s*:\s*(?:white|black|red|green|blue|gray|grey|orange|yellow|purple|pink|brown)\b/i;
...
test('keeps component colors behind shared theme tokens', async () => {
  ...
  assert.deepEqual(violations, [], 'put color values in src/lib/shared/theme/tokens.css instead of components');
});
...
test('overrides xterm viewport defaults with the active terminal theme', async () => {
  const terminalViewport = await readFile(
    join(sourceRoot, 'lib', 'features', 'terminal', 'TerminalViewport.svelte'),
    'utf8'
  );
...
test('gives xterm the resolved shared font stack and the browser language', async () => {
  ...
  const terminalRuntime = await readFile(
    join(sourceRoot, 'lib', 'features', 'terminal', 'terminal-runtime.ts'),
    'utf8'
  );

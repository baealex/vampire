...
import type { SessionActivityState } from '../src/lib/session/view.ts';

function session(
	lastOutputAt: number | null,
	id = 'workspace-1',
	state: ManagedSession['state'] = 'running'
): ManagedSession {
	return {
		id,
...
function terminal(
	...
	name: string,
	lastOutputAt: number,
	foregroundProcess: SessionTerminal['foregroundProcess']
): SessionTerminal {
	return {
		id,
		index,
...
test('uses an explicit agent working signal across silent output gaps', () => {
	const current: ManagedSession = { ...session(2_000), agentState: 'working' };
	assert.equal(view.sessionActivityState(current, activity(current.id, 0, 0), 60_000), 'active');
	assert.equal(view.sessionActivityState({ ...current, agentState: 'waiting' }, activity(current.id, 0, 0), 60_000), 'review');
});

test('places active sessions above review sessions', () => {
	const states: SessionActivityState[] = ['active', 'review', 'idle', 'ended'];
	assert.deepEqual(
...
test('keeps manual workspace order stable while activity changes', () => {
	const sessions = [
		session(1_000, 'workspace-a'),
		session(2_000, 'workspace-b'),
		session(3_000, 'workspace-c')
	];
	assert.deepEqual(
		view.sortSessions(sessions, 'manual', ['workspace-c', 'workspace-a', 'workspace-b']).map((current) => current.id),
		['workspace-c', 'workspace-a', 'workspace-b']
	);
});

test('tracks main session output without treating background process output as agent activity', () => {
	...
	assert.deepEqual(view.sessionProcess(current), { kind: 'command', label: 'codex' });
	assert.equal(view.sessionActivityState(current, activity(current.id, 0, 2_000), 6_000), 'idle');
	assert.equal(view.latestSessionOutputAt(current), 2_000);
});

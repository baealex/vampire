...
} from '../src/lib/features/terminal/server/process-cleanup.ts';
...
function processRecord(pid: number, ppid: number, pgid: number, tty: string, command: string): ProcessRecord {
...
test('reports process groups that remain after forced termination', async () => {
  const initial = processTable([processRecord(400, 10, 400, 'ttys004', 'stuck-process')]);
  const dependencies: ProcessTerminationDependencies = {
...
test('continues process cleanup when releasing the terminal fails', async () => {
  const initial = processTable([processRecord(500, 10, 500, 'ttys005', 'pnpm')]);
  let forceKilled = false;
  const dependencies: ProcessTerminationDependencies = {
    readProcesses: async () => (forceKilled ? new Map() : initial),

<script lang="ts">
	...
	import type { RepositoryChange, RepositorySelection, RepositorySnapshot } from './types';
...
<style>
	...
	.change-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: stretch; width: 100%; min-width: 0; border-bottom: 1px solid var(--color-divider-subtle); background: transparent; color: inherit; }
	...
	.change-open { display: grid; grid-template-columns: 1.6rem minmax(0, 1fr); align-items: start; gap: 0.65rem; min-width: 0; padding: 0.72rem 0.35rem 0.72rem 0.85rem; border: 0; background: transparent; color: inherit; text-align: left; cursor: pointer; }
	.discard-change { align-self: center; display: grid; place-items: center; width: 2rem; height: 2rem; margin-right: 0.45rem; padding: 0; border: 0; border-radius: var(--radius-sm); background: transparent; color: var(--color-text-tertiary); cursor: pointer; opacity: 0; }
	.change-row:hover .discard-change, .change-row:focus-within .discard-change { opacity: 1; }
	.discard-change:hover, .discard-change:focus-visible { background: var(--color-danger-surface-hover); color: var(--color-danger-text); }

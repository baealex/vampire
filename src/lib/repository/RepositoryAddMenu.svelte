...
<style>
	:global(.repository-add-trigger) { display: grid; place-items: center; width: var(--control-height-md); height: var(--control-height-md); padding: 0; border: 1px solid transparent; border-radius: var(--radius-control); background: transparent; color: var(--color-text-secondary); cursor: pointer; }
	:global(.repository-add-trigger:hover), :global(.repository-add-trigger:focus-visible), :global(.repository-add-trigger[data-state='open']) { border-color: var(--color-border); background: var(--color-surface-selected); color: var(--color-text); }
</style>

...
<style>
	.code-editor { display: flex; flex-direction: column; width: 100%; height: 100%; min-width: 0; min-height: 0; background: var(--color-code-background); color: var(--color-text); }
	...
	.save-button:hover:not(:disabled) { border-color: var(--color-accent-hover); background: var(--color-accent-hover); }
	.save-button:disabled { cursor: default; opacity: 0.5; }
	.editor-error { flex: 0 0 auto; margin: 0; padding: 0.4rem 0.7rem; border-top: 1px solid var(--color-danger-border); background: var(--color-danger-surface); color: var(--color-danger-text); font-size: var(--text-caption); line-height: var(--leading-ui); }

	@media (max-width: 40rem) {
		.editor-status { gap: 0.5rem; padding-inline: 0.5rem; }

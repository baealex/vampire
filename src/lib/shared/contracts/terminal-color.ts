...
function rgbColor(red: string, green: string, blue: string): string {
  return `#${[red, green, blue].map((component) => componentToByte(component).toString(16).padStart(2, '0')).join('')}`;
}
...
export function terminalThemeColor(slot: TerminalColorSlot, theme: TerminalThemeColors, reportedColor: string): string {
  const themeColor = slot === 10 ? theme.foreground : slot === 11 ? theme.background : theme.cursor;

...
async function confirmSelectedAction() {
  ...
  confirmError = '';
  const result = selectedAction === 'close' ? await closeSession(session) : await remove(session);
  if (!result.ok) {
    confirmError =
      result.error || `Unable to ${selectedAction === 'close' ? 'close the session' : 'remove the workspace'}.`;
    return;
  }
  confirming = undefined;
  onOpenChange(false);
  ...
  {#snippet children()}
    ...
    {#if confirming}
      <div
        ...
        role="group"
        aria-label={confirming === 'close' ? 'Confirm closing session' : 'Confirm removing workspace'}
      >
        ...
        <p>
          ...
          {:else if isWorktreeWorkspace(session)}
            {session.state === 'running' ? 'The running shell will stop. ' : ''}The managed working copy and any

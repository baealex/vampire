<script lang="ts">
	...
	const hasUnsavedChanges = $derived(
		JSON.stringify(editableProfiles) !== JSON.stringify(profiles)
			|| selectedProfileId !== session.startupProfileId
	);
		...
		<div class="startup-profile-dialog">
			<div class="dialog-intro">
				<div>
					...
					<p>Create a command here or reuse one saved from another workspace. Profiles are shared on this Vampire server; only the startup selection belongs to this workspace.</p>
				</div>
				<button class="add-button" type="button" onclick={addProfile} disabled={editableProfiles.length >= MAX_LAUNCH_PROFILES}>
			...
			<label class:selected={selectedProfileId === null} class="no-startup-option">
				<input type="radio" name="startup-profile" checked={selectedProfileId === null} onchange={() => selectedProfileId = null} />
				<span>
					...
					{#each editableProfiles as profile, index (profile.id)}
						<article class:selected={selectedProfileId === profile.id} class="profile-card">
							...
							<div class="profile-card__footer">
								<span>{selectedProfileId === profile.id ? 'Selected for this workspace' : 'Reusable in any workspace'}</span>
								<label class="profile-selection">
									<input type="radio" name="startup-profile" checked={selectedProfileId === profile.id} onchange={() => selectedProfileId = profile.id} />
									<span>Use here</span>
								</label>
							</div>
						</article>
			...
			{#if savingError}<p class="feedback" role="alert">{savingError}</p>{/if}
		</div>
	{/snippet}
...
<style>
	.startup-profile-dialog { display: grid; gap: 0.9rem; min-width: 0; }
	...
	.dialog-intro p { max-width: 35rem; margin: 0.25rem 0 0; color: var(--color-text-secondary); font-size: var(--text-caption); line-height: var(--leading-body); }
	...
	.add-button:hover:not(:disabled) { border-color: var(--color-accent); color: var(--color-accent); }
	.add-button:disabled { cursor: default; opacity: 0.5; }
	.no-startup-option { display: grid; grid-template-columns: auto minmax(0, 1fr); align-items: start; gap: 0.65rem; padding: 0.7rem 0.75rem; border: 1px solid var(--color-border); border-radius: var(--radius-md); background: var(--color-surface-raised); cursor: pointer; }
	.no-startup-option.selected { border-color: var(--color-accent); background: var(--color-surface-selected); }
	.no-startup-option input { margin-top: 0.18rem; accent-color: var(--color-accent); }
	.no-startup-option > span { display: grid; gap: 0.2rem; }
	.no-startup-option strong { color: var(--color-text); font-size: var(--text-caption); font-weight: var(--weight-medium); }
	.no-startup-option small { color: var(--color-text-tertiary); font-size: var(--text-nano); }
	...
	.profile-card.selected { border-color: var(--color-accent); }
	...
	.profile-card input:not([type="radio"]) { width: 100%; min-height: 2.3rem; padding: 0 0.6rem; border: 1px solid var(--color-border-strong); border-radius: var(--radius-sm); background: var(--color-control-background); color: var(--color-text); font: inherit; font-size: var(--text-caption); }
	.profile-card input:focus-visible, .no-startup-option input:focus-visible { outline: 2px solid var(--color-accent); outline-offset: 2px; }
	...
	.profile-card__footer { display: flex; align-items: center; justify-content: space-between; gap: 0.75rem; color: var(--color-text-tertiary); font-size: var(--text-nano); }
	.profile-selection { display: inline-flex !important; grid-template-columns: none !important; flex: 0 0 auto; align-items: center; gap: 0.4rem !important; color: var(--color-text-secondary) !important; cursor: pointer; }
	.profile-selection input { accent-color: var(--color-accent); }
	.feedback { margin: 0; padding: 0.65rem 0.75rem; border-radius: var(--radius-sm); background: var(--color-danger-surface-hover); color: var(--color-danger-text); font-size: var(--text-caption); }
	.dialog-footer { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
	.dialog-footer p { margin: 0; color: var(--color-text-tertiary); font-size: var(--text-nano); line-height: var(--leading-body); }
	...
	.save-button:disabled { cursor: default; opacity: 0.55; }

	@media (max-width: 38rem) {
		.dialog-intro, .empty-profiles, .dialog-footer { align-items: flex-start; flex-direction: column; }

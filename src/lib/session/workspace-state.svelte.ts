...
import { maxTimestamp, reconcileSessionOrder, sortSessions, type SessionActivityRecord } from './view.ts';
...
export class SessionWorkspaceState {
  ...
  displayedSessions = $derived(
    sortSessions(this.sessions, this.sessionOrderMode, this.manualSessionOrder, this.activityOrder)
  );
  shortcutSessions = $derived(this.displayedSessions.filter((session) => session.state === 'running'));
  activeSession = $derived(
    this.requestedSessionId ? this.sessions.find((session) => session.id === this.requestedSessionId) : undefined
  );
  ...
  applyLaunchProfiles(launchProfiles: LaunchProfile[]) {
    ...
    this.sessions = this.sessions.map((session) =>
      session.startupProfileId && !profileIds.has(session.startupProfileId)
  ...
  async updateSessionNote(sessionId: string, note: string) {
    ...
    this.sessions = this.sessions.map((session) =>
      session.id === sessionId ? { ...session, notePreview: data.notePreview } : session
    );
  }

  async updateWorkspaceAlias(sessionId: string, alias: string): Promise<{ ok: boolean; error?: string }> {
    try {
      ...
      this.sessions = this.sessions.map((session) =>
        session.id === sessionId ? { ...session, workspaceLabel: data.alias ?? '' } : session
      );
      return { ok: true };
    } catch (error) {
      if (isUnauthorized(error)) this.#options.onUnauthorized();
  ...
  async loadSessionNote(sessionId: string, refresh = false): Promise<string> {
    ...
    const request = requestJson<{ note: string }>(`/api/sessions/${encodeURIComponent(sessionId)}/note`, {
      cache: 'no-store',
    })
      .then(({ note }) => {
    ...
    try {
      ...
      this.sessions = this.sessions.map((session) =>
        session.id === sessionId
          ? {
              ...session,
              terminals: this.#backgroundTerminals.applyStarted(sessionId, session.terminals, data.backgroundProcess),
            }
          : session
      );
  ...
  async favoriteBackgroundCommand(sessionId: string, command: string): Promise<boolean> {
    ...
    try {
      ...
      this.sessions = this.sessions.map((session) =>
        session.id === sessionId ? { ...session, favoriteCommands: data.favoriteCommands } : session
      );
      return true;
    } catch (error) {
      if (isUnauthorized(error)) this.#options.onUnauthorized();
  ...
  recordSessionInput(sessionId: string, timestamp: number) {
    this.sessions = this.sessions.map((session) =>
      session.id === sessionId ? { ...session, lastActiveAt: timestamp } : session
    );
    ...
    this.#activityRequestTimers.set(
      ...
      window.setTimeout(() => {
        ...
        void requestJson<{ lastActiveAt: number }>(`/api/sessions/${encodeURIComponent(sessionId)}`, {
          method: 'PATCH',
        })
          .then(({ lastActiveAt }) => {
            this.sessions = this.sessions.map((session) =>
  ...
  openSession(session: ManagedSession) {
    ...
    if (previousSessionId && previousSessionId !== session.id && this.#options.isSessionObserved(previousSessionId)) {
  ...
  async restartSession(session: ManagedSession, launchProfileId?: string | null): Promise<boolean> {
  ...
  async removeSession(session: ManagedSession): Promise<boolean> {
    ...
    this.backgroundActionErrorSessionId = undefined;
    try {
      await requestJson<{ ok: boolean }>(`/api/sessions/${encodeURIComponent(session.id)}?terminate=true`, {
        method: 'DELETE',
      });
      this.#backgroundTerminals.clearSession(session.id);
      this.invalidateSessions();

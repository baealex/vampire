...
const selectedEndedSession = $derived(
  displayedSessions.some(
...
async function openSessionDialog(session: ManagedSession, callback: (session: ManagedSession) => void) {
        ...
        <span class="session-summary">
          ...
          {#if isWorktreeWorkspace(session)}
            <span
              ...
              title={`${repositoryName}${session.worktreeBranch ? ` · ${session.worktreeBranch}` : ' · Git worktree'}`}
            >
          ...
          <span class="agent-summary" title={sessionActivityHint(session, activityRecords, now)}>
            ...
            {#if process}
              <span
                ...
                style={`--session-program-color: ${sessionProcessColor(process)}`}
                title={sessionProcessHint(session)}
                >{process.label}</span
              >
              <span class="session-context-divider" aria-hidden="true">·</span>
            {/if}
            ...
            {/if}
            <time
              datetime={new Date(latestSessionOutputAt(session)).toISOString()}
              title={`Main terminal update ${new Date(latestSessionOutputAt(session)).toLocaleString()}`}
              >{formatSessionTimestamp(latestSessionOutputAt(session), now)}</time
            >
          </span>
...
.session-group.working .session-group-header {
  color: var(--color-warning-accent);
}
.session-group.review .session-group-header {
  color: var(--color-info-text);
}
.session-group.idle .session-group-header {
...
.session-group-toggle :global(svg) {
  margin-left: auto;
  transition: transform 150ms ease;
}
.session-group-toggle :global(svg.expanded) {
...
@media (hover: hover) {
  ...
  .session-row-shell.selected .session-row:hover {
    background: var(--color-surface-active-hover);
  }
}
.session-row-shell.selected {
  background: var(--color-surface-active);
  box-shadow: inset 0.18rem 0 var(--color-accent);
}
.session-row-shell[draggable="true"] {
  cursor: grab;
}
.session-row-shell.dragging {
  opacity: 0.42;
  cursor: grabbing;
}
.session-row-shell.dropBefore::before,
.session-row-shell.dropAfter::after {
  position: absolute;
  z-index: 4;
  right: 0.65rem;
  left: 0.65rem;
...
.session-program {
  flex: 0 0 auto;
  color: var(--session-program-color, var(--color-text-secondary));
  font-weight: var(--weight-medium);
}
.workspace-state {
  flex: 0 0 auto;
...
.session-note-preview :global(svg) {
  flex: 0 0 auto;
}
.session-note-preview span {
  min-width: 0;
  overflow: hidden;
  text-overflow: ellipsis;
}
.session-row.missing .session-summary {
  opacity: 0.62;
}
.empty-state {
  display: grid;

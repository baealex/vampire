...
const LEGACY_STATUS_PLUGIN_CPU_COMMAND = [
...
const LEGACY_STATUS_PLUGIN_MEMORY_COMMAND = [
...
function migrateStatusPlugins(value: unknown): StatusPlugin[] | undefined {
	...
	const migrated = value.map((plugin): unknown => {
		...
		if (fields.type === 'system'
			...
			&& Object.hasOwn(fields, 'metric')) {
			command = fields.metric === 'cpu'
				? STATUS_PLUGIN_CPU_COMMAND
				: fields.metric === 'memory'
					? STATUS_PLUGIN_MEMORY_COMMAND
					: undefined;
		} else if (fields.type === 'command' && typeof fields.command === 'string') {
			command = fields.command === LEGACY_STATUS_PLUGIN_CPU_COMMAND
				? STATUS_PLUGIN_CPU_COMMAND
				: fields.command === LEGACY_STATUS_PLUGIN_MEMORY_COMMAND
					? STATUS_PLUGIN_MEMORY_COMMAND
...
export async function writeStatusPluginStore(
	state: StatusPluginStore,

...
import { createGitWorktree, GitWorktreeError, removeManagedGitWorktree, rollbackGitWorktree } from './git-worktree.ts';
import type { AgentState } from '../session/agent.ts';
import { isLaunchProfileList, normalizeLaunchProfiles } from '../session/launch-profiles.ts';
...
export type SessionMutationErrorReason =
  | 'not-found'
  | 'session-running'
  | 'session-not-running'
...
function reconcileWorkspacePreferences(
  ...
  const manualSessionOrder = [...new Set(preferences.manualSessionOrder)].filter((id) => sessionIds.has(id));
...
function validateWorkspacePreferences(input: WorkspacePreferences): WorkspacePreferences {
  if (
    (input.sessionOrderMode !== 'activity' && input.sessionOrderMode !== 'manual') ||
...
export async function readManagedWorkspacePreferences(): Promise<WorkspacePreferences | null> {
  ...
  return state.workspacePreferences ? reconcileWorkspacePreferences(state.sessions, state.workspacePreferences) : null;
}
...
export async function updateManagedWorkspacePreferences(input: WorkspacePreferences): Promise<WorkspacePreferences> {
  return exclusively(async () => {
    const state = await readState();
    const workspacePreferences = reconcileWorkspacePreferences(state.sessions, validateWorkspacePreferences(input));
...
export async function createManagedWorktreeSession(input: {
...
export async function restartManagedSession(
  id: string,
...
function resolveRestartProfileId(
  ...
  if (!selectedProfileId || !launchProfiles.some((profile) => profile.id === selectedProfileId)) {
    throw new SessionMutationError('invalid-startup-profile', 'The launch profile was not found.');
  }
...
export async function captureManagedBackgroundOutput(id: string, terminalId: string): Promise<string> {
  ...
  const running = (await listTmuxSessions()).find((session) => session.name === stored.tmuxSession);
  if (!running)
    throw new SessionMutationError('session-not-running', 'Reopen the workspace before reading background output.');
  const backgroundProcess = running.terminals.slice(1).find((candidate) => candidate.id === terminalId);
  if (!backgroundProcess)
    throw new SessionMutationError('background-not-found', 'Background process was not found in this workspace.');

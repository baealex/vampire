...
import { parseWorkspaceRootPaths } from './runtime-config.ts';
import { pathStaysInside } from './path-policy.ts';
 ...
 *
 */
export async function resolveWorkspaceDirectory(cwd: string, roots: WorkspaceRoot[]): Promise<{ root: WorkspaceRoot; path: string }> {

...
export type GitWorktreeErrorReason =
	| 'invalid-name'
	| 'not-git'
	| 'no-head'
	| 'invalid-location'
...
function normalizeLabel(value: string): string {
	...
	if (!label || label.length > WORKTREE_LABEL_MAX_LENGTH || /[\0-\x1f\x7f]/u.test(label)) {
		throw worktreeError('invalid-name', `Task name must be a single line between 1 and ${WORKTREE_LABEL_MAX_LENGTH} characters.`);
	}
	return label;
}
...
export async function createGitWorktree(
	...
	const requestedManagedRoot = resolve(options.managedRoot ?? join(dirname(sessionStatePath()), 'worktrees'));
	...
	if (pathStaysInside(sourceRoot, managedRoot)) {
		throw worktreeError('invalid-location', 'VAMPIRE_STATE_DIR must be outside the source Git working tree to create isolated workspaces.');
	}

	const repositoryName = (basename(commonDirectory) === '.git' ? basename(dirname(commonDirectory)) : basename(sourceRoot)) || 'repository';
...
export async function removeManagedGitWorktree(
	...
	if (listedWorktreePaths(before.stdout).includes(targetLocation.path)) {
		await runGit(
			gitCwd,
			['worktree', 'remove', '--force', targetLocation.path],
			[0],
			'remove'
		);

...
function shouldRefreshForWorkspaceEvent(filename: WatchFilename): boolean {
  ...
  return path === '.git/HEAD' || path === '.git/index' || path === '.git/packed-refs' || path.startsWith('.git/refs/');
}
...
class RepositoryStatusHub {
  ...
  async observe(socket: RepositoryStatusSocket, workspaceId: string): Promise<void> {
...
export function observeRepositoryStatus(socket: RepositoryStatusSocket, workspaceId: string): Promise<void> {
  return repositoryStatusHub.observe(socket, workspaceId);
}

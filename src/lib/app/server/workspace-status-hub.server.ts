...
export interface WorkspaceStatusSubscriber {
  close(reason: string): void;
  send(payload: WorkspaceServerMessage): boolean;
}
...
class WorkspaceStatusHub {
  #clients = new Set<WorkspaceStatusSubscriber>();
  ...
  async subscribe(client: WorkspaceStatusSubscriber): Promise<() => void> {
    try {
      await this.#refresh();
      if (
        !client.send({
      ...
      if (firstClient) {
        if (statusWidgetCommandsAllowed()) {
          try {
            await this.#statusPlugins.start();
          } catch {
            client.send({ type: 'error', message: 'Unable to load status plugins.' });
          }
        } else {
          client.send({ type: 'status-plugins-snapshot', plugins: [] });
        }
      } else {
        client.send({ type: 'status-plugins-snapshot', plugins: this.#statusPlugins.snapshots() });
      }
      this.#startPolling();
      return () => this.unsubscribe(client);
    } catch {
      client.send({ type: 'error', message: 'Unable to load workspaces.' });
      client.close('workspace state unavailable');
      return () => undefined;
    }
  }

  unsubscribe(client: WorkspaceStatusSubscriber): void {
  ...
  close(): void {
    ...
    this.#pendingAgentStates.clear();
    this.#statusPlugins.stop();
    for (const client of this.#clients) client.close('server shutting down');
    this.#clients.clear();
  }
  ...
  #broadcast(payload: WorkspaceServerMessage, excludedClient?: WorkspaceStatusSubscriber): void {
    for (const client of this.#clients) {
      if (client === excludedClient) continue;
      if (!client.send(payload)) {
        this.unsubscribe(client);
        client.close('workspace subscriber fell behind');
      }
    }
  }
}
...
export function subscribeWorkspaceStatus(subscriber: WorkspaceStatusSubscriber): Promise<() => void> {
  return workspaceStatusHub.subscribe(subscriber);
}

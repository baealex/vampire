...
<style>
	...
	.terminal-control-handoff { position: absolute; z-index: 4; top: 0.75rem; right: 0.75rem; display: flex; align-items: center; gap: 0.5rem; max-width: calc(100% - 1.5rem); padding: 0.38rem; border: 1px solid var(--color-border); border-radius: 0.78rem; background: var(--color-surface-overlay); box-shadow: var(--shadow-popover); color: var(--color-text-secondary); font-size: var(--text-label); line-height: var(--leading-ui); white-space: nowrap; }
	.terminal-status-icon { display: grid; flex: 0 0 2rem; place-items: center; width: 2rem; height: 2rem; border-radius: 0.58rem; }
	.terminal-status-icon.control { background: var(--color-accent-soft); color: var(--color-accent-soft-text); }
	.terminal-control-label { padding: 0 0.18rem 0 0.05rem; }
	.terminal-control-handoff button, .terminal-status-card button { flex: 0 0 auto; min-height: 2.25rem; padding: 0 0.72rem; border: 1px solid var(--color-border); border-radius: 0.55rem; background: var(--color-surface-selected); color: var(--color-text); font: inherit; font-weight: var(--weight-medium); cursor: pointer; touch-action: manipulation; }
	.terminal-control-handoff button:hover, .terminal-status-card button:hover { border-color: var(--color-border-strong); background: var(--color-control-hover); }
	.terminal-control-handoff button:focus-visible, .terminal-status-card button:focus-visible { outline: none; box-shadow: var(--shadow-accent-focus); }
	.terminal-status-card { position: absolute; z-index: 5; right: 0.75rem; bottom: 0.75rem; display: grid; grid-template-columns: auto minmax(0, 1fr) auto; align-items: center; gap: 0.55rem; width: max-content; max-width: min(30rem, calc(100% - 1.5rem)); padding: 0.45rem; border: 1px solid var(--color-border-strong); border-radius: 0.78rem; background: var(--color-surface-overlay); box-shadow: var(--shadow-popover); color: var(--color-text-secondary); font-size: var(--text-label); line-height: var(--leading-ui); }
	.terminal-status-message { min-width: 0; padding: 0 0.2rem 0 0.05rem; text-align: left; }
	.terminal-status-icon.error { background: var(--color-danger-surface); color: var(--color-danger-text); }
	.terminal-error { border-color: var(--color-danger-border); }
	.terminal-error button { border-color: var(--color-danger-border); color: var(--color-danger-text-strong); }
	.terminal-status-icon.reconnecting { background: var(--color-warning-surface); color: var(--color-warning-accent); }
	.terminal-connection-status .terminal-status-icon :global(svg) { animation: terminal-status-spin 0.9s linear infinite; }
	.image-paste-notice { display: flex; align-items: center; justify-content: center; gap: 0.75rem; margin: 0; padding: 0.45rem 0.75rem; font-size: var(--text-label); line-height: var(--leading-ui); text-align: center; }
	...
	@media (max-width: 32rem) {
		.terminal-control-handoff { top: 0.5rem; right: 0.5rem; width: auto; max-width: calc(100% - 1rem); }
		.terminal-control-label { flex: 0 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
		.terminal-control-handoff button, .terminal-status-card button { min-height: 2.75rem; }
		.terminal-status-card { right: 0.5rem; bottom: 0.5rem; width: calc(100% - 1rem); max-width: none; }
	}

	@keyframes terminal-status-spin { to { transform: rotate(360deg); } }

	@media (prefers-reduced-motion: reduce) {
		.terminal :global(.xterm) { transition: none; }
		.terminal-connection-status .terminal-status-icon :global(svg) { animation: none; }
	}
</style>

<script lang="ts">
	...
	function intervalMilliseconds(): number {
		const multiplier = intervalUnit === 'days'
			? 24 * 60 * 60_000
			: intervalUnit === 'hours' ? 60 * 60_000 : 60_000;
	...
	function scheduleInput(): SessionAutomationSchedule | undefined {
		...
		if (!Number.isFinite(intervalMs)
			...
			|| intervalMs > MAX_AUTOMATION_INTERVAL_MS) return undefined;
	...
	function statusLabel(automation: SessionAutomation): string {
		...
		if (!automation.enabled) {
			return automation.schedule.type === 'once' && automation.lastOutcome === 'submitted'
				? 'Sent'
				: 'Paused';
		}
...
<div class:embedded class="automation-panel" role={embedded ? undefined : 'dialog'} aria-labelledby={embedded ? undefined : 'automation-panel-title'}>
	{#if !embedded}
		<header>
			<div>
	...
	<form onsubmit={(event) => { event.preventDefault(); void createAutomation(); }}>
		<label>
			<span>Name</span>
			<input bind:value={name} maxlength={SESSION_AUTOMATION_NAME_MAX_LENGTH} placeholder="Daily project check" required />
		</label>
		<label>
			<span>Prompt</span>
			<textarea bind:value={prompt} maxlength={SESSION_AUTOMATION_PROMPT_MAX_LENGTH} rows="4" placeholder="Review the current work and take the next useful step…" required></textarea>
		</label>
	...
	{#if mutationError}<p class="automation-error" role="alert">{mutationError}</p>{/if}
		...
		{:else}
			{#each automations as automation (automation.id)}
				<article class:paused={!automation.enabled}>
					...
					{#if automation.lastRunAt}<p class="automation-meta">Last sent {formatTimestamp(automation.lastRunAt)}</p>{/if}
					{#if automation.lastError}<p class="automation-error" role="alert">{automation.lastError}</p>{/if}
					<div class="automation-actions">
						...
						>
							{#if automation.enabled}<Pause size={14} strokeWidth={1.9} aria-hidden="true" /> Pause{:else}<Play size={14} strokeWidth={1.9} aria-hidden="true" /> {resumeLabel(automation)}{/if}
						</button>
						<button
							type="button"
							class="delete-button"
...
<style>
	.automation-panel { display: grid; gap: 1rem; width: 100%; max-height: min(46rem, calc(100dvh - 5rem)); box-sizing: border-box; overflow-y: auto; padding: 1rem; border: 1px solid var(--color-border-strong); border-radius: 0.8rem; background: var(--color-surface-overlay); box-shadow: var(--shadow-popover); }
	.automation-panel.embedded { max-height: none; padding: 0; border: 0; border-radius: 0; background: transparent; box-shadow: none; }
	.automation-description { margin: 0; color: var(--color-text-secondary); font-size: var(--text-caption); line-height: var(--leading-ui); }
	...
	form { display: grid; gap: 0.7rem; padding: 0.8rem; border: 1px solid var(--color-border); border-radius: 0.65rem; background: var(--color-surface-sunken); }
	label { display: grid; gap: 0.32rem; min-width: 0; color: var(--color-text-secondary); font-size: var(--text-caption); font-weight: var(--weight-medium); }
	input, textarea, select { width: 100%; min-width: 0; box-sizing: border-box; padding: 0.6rem 0.65rem; border: 1px solid var(--color-border); border-radius: var(--radius-sm); outline: none; background: var(--color-field-background); color: var(--color-text); font: inherit; font-size: var(--text-label); }
	textarea { resize: vertical; line-height: var(--leading-body); }
	input:focus, textarea:focus, select:focus { border-color: var(--color-accent); box-shadow: var(--shadow-accent-focus); }
	.schedule-row, .interval-row { display: grid; grid-template-columns: minmax(0, 0.72fr) minmax(0, 1.28fr); gap: 0.65rem; }
	.interval-row { grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); }
	.create-button { display: inline-flex; align-items: center; justify-content: center; gap: 0.4rem; min-height: 2.45rem; padding: 0 0.8rem; border: 0; border-radius: var(--radius-control); background: var(--color-accent); color: var(--color-accent-ink); font: inherit; font-size: var(--text-label); font-weight: var(--weight-medium); cursor: pointer; }
	.create-button:hover:not(:disabled) { background: var(--color-accent-hover); }
	.create-button:disabled { cursor: wait; opacity: 0.65; }
	.automation-list { display: grid; gap: 0.65rem; }
	.automation-list article { display: grid; gap: 0.55rem; padding: 0.75rem; border: 1px solid var(--color-border); border-radius: 0.65rem; background: var(--color-control-background); }
	.automation-list article.paused { opacity: 0.72; }
	.automation-heading { display: flex; align-items: flex-start; justify-content: space-between; gap: 0.75rem; }
	.automation-heading > div { display: grid; gap: 0.15rem; min-width: 0; }
	.automation-heading strong { overflow: hidden; color: var(--color-text); font-size: var(--text-label); text-overflow: ellipsis; white-space: nowrap; }
	.automation-heading span, .automation-meta { color: var(--color-text-tertiary); font-size: var(--text-caption); line-height: var(--leading-ui); }
	.automation-heading :global(svg) { flex: 0 0 auto; color: var(--color-command); }
	.automation-prompt { display: -webkit-box; margin: 0; overflow: hidden; color: var(--color-text-secondary); font-size: var(--text-label); line-height: var(--leading-body); -webkit-box-orient: vertical; -webkit-line-clamp: 3; line-clamp: 3; }
	.automation-meta, .automation-error { margin: 0; }
	.automation-error { color: var(--color-danger-text); font-size: var(--text-caption); line-height: var(--leading-ui); }
	.automation-actions { display: flex; justify-content: flex-end; gap: 0.45rem; }
	.automation-actions button, .automation-empty button { display: inline-flex; align-items: center; justify-content: center; gap: 0.32rem; min-height: 2.15rem; padding: 0 0.65rem; border: 1px solid var(--color-border); border-radius: var(--radius-sm); background: transparent; color: var(--color-text-secondary); font: inherit; font-size: var(--text-caption); cursor: pointer; }
	.automation-actions button:hover:not(:disabled), .automation-empty button:hover { background: var(--color-control-hover); color: var(--color-text); }
	.automation-actions .delete-button { color: var(--color-danger-text); }
	.automation-actions button:disabled { cursor: wait; opacity: 0.55; }
	.automation-empty { margin: 0; padding: 1rem; border: 1px dashed var(--color-border); border-radius: 0.65rem; color: var(--color-text-tertiary); font-size: var(--text-label); text-align: center; }
	.automation-empty p { margin: 0 0 0.65rem; }

	@media (max-width: 32rem) {
		.automation-panel { max-height: calc(100dvh - 4rem); padding: 0.85rem; }
		.schedule-row, .interval-row { grid-template-columns: 1fr; }

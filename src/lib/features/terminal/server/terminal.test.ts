...
test('uses physical captures for visible screens so styled trailing cells survive', () => {
  const mainScreen = terminalPaneState('0\t4294967295\t4294967295\t0\t1\t17\t4\t0\t0\t0\t0\t1\t0\t23\n', {
    columns: 80,
    rows: 24,
  });
  const alternateScreen = terminalPaneState('1\t3\t2\t1\t1\t17\t4\t0\t1\t0\t0\t1\t0\t23\n', {
    columns: 80,
    rows: 24,
  });
  assert.ok(mainScreen);
  assert.ok(alternateScreen);
  assert.equal(terminalCaptureFlag(mainScreen, 0), '-N');
  assert.equal(terminalCaptureFlag(mainScreen, 40), '-J');
  assert.equal(terminalCaptureFlag(alternateScreen, 0), '-N');
  assert.equal(terminalCaptureFlag(alternateScreen, 40), '-N');
});

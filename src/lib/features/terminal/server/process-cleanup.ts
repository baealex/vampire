...
export async function listProcesses(): Promise<Map<number, ProcessRecord>> {
  const { stdout } = await execFile('ps', ['-axo', 'pid=,ppid=,pgid=,tpgid=,tty=,state=,command='], {
...
class OwnedProcessTracker {
  ...
  constructor(rootProcessIds: Iterable<number>) {
    this.#ownedProcessIds = new Set([...rootProcessIds].filter((pid) => Number.isInteger(pid) && pid > 1));
  }
  ...
  activeProcessGroupIds(processes: Map<number, ProcessRecord>): number[] {
    ...
    while (discovered) {
      ...
      for (const process of processes.values()) {
        ...
        const attachedToOwnedTerminal = hasControllingTerminal(process.tty) && this.#ownedTerminals.has(process.tty);
    ...
    try {
      ...
      if (remainingAfterTerm.length > 0) {
        ...
        if (remainingAfterKill.length > 0) {
          terminationErrors.push(new Error(`Workspace process groups did not stop: ${remainingAfterKill.join(', ')}`));
        }
      }
    } catch (error) {
      terminationErrors.push(error);

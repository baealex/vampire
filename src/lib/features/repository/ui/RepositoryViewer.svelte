...
$effect(() => {
  ...
  const selectionKey = `${workspaceId}:${requestedSelection.kind}:${requestedSelection.path}`;
    ...
    {:else if selection.kind === 'file' && file && !imagePreview}
      <div class="file-document">
        ...
        {:then loadedEditor}
          ...
          <RepositoryCodeEditor {workspaceId} {file} onSaved={handleFileSaved} onDirtyChange={setFileDirty} />
        {:catch}

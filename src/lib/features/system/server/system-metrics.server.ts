...
import type { SystemMetrics } from '~/lib/shared/contracts/system-metrics.ts';
...
function readMemoryUsage(): Pick<SystemMetrics, 'memoryUsedBytes' | 'memoryTotalBytes'> {
  ...
  const memoryAvailableBytes = typeof process.availableMemory === 'function' ? process.availableMemory() : freemem();

...
import type { ListeningPort, ListeningPortTermination } from '~/lib/shared/contracts/listening-ports.ts';
...
function commandErrorCode(cause: unknown): string | number | undefined {
  return cause && typeof cause === 'object' && 'code' in cause ? (cause as { code?: string | number }).code : undefined;
}
...
async function runCommand(file: string, args: string[]): Promise<string> {
  try {
    ...
    if (file === 'lsof' && commandErrorCode(cause) === 1) {
      const stdout =
        cause && typeof cause === 'object' && 'stdout' in cause ? (cause as { stdout?: unknown }).stdout : '';
...
async function readListeningSockets(platform: NodeJS.Platform, run: CommandRunner): Promise<ListeningSocket[]> {
  if (platform === 'darwin') {
    return parseLsofListeningSockets(await run('lsof', ['-nP', '-iTCP', '-sTCP:LISTEN', '-F0pcPn']));
  }
...
export async function listListeningPorts(): Promise<ListeningPort[]> {
  ...
  try {
    ...
    if (commandErrorCode(cause) === 'ENOENT') {
      throw new ListeningPortError('tool-unavailable', 'Vampire could not find lsof or ss to inspect listening ports.');
    }

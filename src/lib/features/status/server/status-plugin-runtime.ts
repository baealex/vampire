...
export interface StatusPluginRuntimeOptions {
  ...
  runCommand?: (command: string, options?: StatusPluginCommandOptions) => Promise<StatusPluginCommandResult>;
  now?: () => number;
...
export class StatusPluginRuntime {
  ...
  constructor(onUpdate: (snapshots: StatusPluginSnapshot[]) => void, options: StatusPluginRuntimeOptions = {}) {
  ...
  #applyConfiguration(plugins: StatusPlugin[], rerunAll: boolean): void {
    ...
    for (const plugin of plugins) {
      ...
      const reusableSnapshot = cached?.executionSignature === signature ? cloneSnapshot(cached.snapshot) : undefined;
      ...
      const elapsedMs =
        reusableSnapshot === undefined ? plugin.intervalMs : Math.max(0, this.#now() - cached!.attemptedAt);
      created.push({
        entry,

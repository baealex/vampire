...
import { readRepositoryCommitDiff, RepositoryReadError } from '~/lib/features/repository/server/repository.server.ts';
...
export const GET: RequestHandler = async (event) => {
  ...
  const hash = event.url.searchParams.get('path');
  if (!hash) throw error(400, 'Commit hash is required.');

  const workspace = await findWorkspaceDirectory(id);
  if (!workspace) throw error(404, 'Workspace was not found.');
  try {
    return json(await readRepositoryCommitDiff(workspace.cwd, hash));
  } catch (cause) {
    if (cause instanceof RepositoryReadError) throw error(repositoryErrorStatus(cause.reason), cause.message);
    throw error(500, 'Vampire could not read this commit.');
  }
};

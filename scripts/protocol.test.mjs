...
} from '../src/lib/terminal/protocol.mjs';
import {
	decodeWorkspaceServerMessage,
	encodeWorkspaceServerMessage
} from '../src/lib/app/workspace-protocol.mjs';
...
test('validates complete workspace messages before applying them to client state', () => {
	const snapshot = { type: 'sessions-snapshot', sessions: [managedSession()] };
	...
	assert.deepEqual(
		decodeWorkspaceServerMessage(encodeWorkspaceServerMessage({
			...
			id: 'session-1',
			changes: { state: 'missing', lastOutputAt: null, foregroundProcess: null }
		})),
		{
			type: 'session-updated',
			id: 'session-1',

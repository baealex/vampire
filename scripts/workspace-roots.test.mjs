...
import {
	...
	WorkspaceRootError
} from '../src/lib/server/workspace-roots.mjs';
...
test('lists immediate real directories and never exposes files or linked directories', async (t) => {
	...
	assert.deepEqual(roots.map(({ path }) => path), [await realpath(root)]);
	...
	assert.deepEqual(listing.directories.map(({ name }) => name), ['.hidden', 'alpha', 'zeta']);

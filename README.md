...
A worker's Result is a claim, not completion. Vampire compares declared paths with the actual Git state, rejects forbidden or unexpected changes, reruns allowlisted verification commands without a shell, and applies the Task approval policy. The crown control beside Background presents assignments as `Preparing → Waiting for agent → Working → Checking result → Waiting for King`, while protocol event names and UUIDs stay in the structured inbox. Workspace handoffs, setup failures, focused questions, and owner-gated Tasks surface there as direct actions. The authenticated owner can approve in one click, adding an optional rationale only when the default audit record is not enough.
...
Attempt delivery is bounded: Vampire escalates a delivered Task that does not report `started` within 2 minutes and a working Task that does not submit a Result within 60 minutes. King UI refreshes time out after 10 seconds, and ordinary control actions or CLI calls fail visibly after 60 seconds instead of hanging forever. Verification commands share one five-minute budget per Attempt; the matching manual CLI request allows a short response grace period beyond that budget. Vampire never silently resends an uncertain prompt. A preparation-only failure can be retried after the owner starts the missing agent, without being presented as an approvable Result. Removing the King workspace cancels active workflow state, returns every checkout to manual control, and retains the structured history for audit.

...
test('creates the singleton King workspace from the browser', async ({ context, page }) => {
  ...
  await expect(page.getByRole('button', { name: /Open running The King of Vampire workspace/ })).toBeVisible();
...
test('adds and moves files through repository menus and drop points', async ({ context, page }) => {
  ...
  try {
    ...
    await expect.poll(async () => (await tmuxPaneRows(workspace.tmuxSession)).join('')).toContain('terminal-drop.txt');
  } finally {
    await Promise.all([
      rm(uploadDirectory, { recursive: true, force: true }),
      rm(rootUpload, { force: true }),

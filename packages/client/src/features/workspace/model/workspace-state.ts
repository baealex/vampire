...
import type {
  ...
  WorkspaceTerminal,
} from '@vampire/lib/shared/contracts/workspace.ts';
import type {
...
export class WorkspaceState {
  workspaces = stateValue<ManagedWorkspace[]>([]);
  launchProfiles = stateValue<LaunchProfile[]>([]);
  defaultStartupProfileId = stateValue<string | null>(null);
  composerHistorySettings = stateValue<WorkspaceComposerHistorySettings>({
    ...DEFAULT_WORKSPACE_COMPOSER_HISTORY_SETTINGS,
  });
  cwd = stateValue('');
  loading = stateValue(false);
  starting = stateValue(false);
  startError = stateValue('');
  newWorkspaceOpen = stateValue(false);
  workspacesLoaded = stateValue(false);
  requestedWorkspaceId = stateValue<string | undefined>(undefined);
  errorMessage = stateValue('');
  workspaceAction = stateValue<'restart' | 'close' | 'remove' | undefined>(undefined);
  workspaceActionError = stateValue('');
  startingBackgroundWorkspaceId = stateValue<string | undefined>(undefined);
  stoppingBackgroundProcessId = stateValue<string | undefined>(undefined);
  updatingFavoriteCommand = stateValue<string | undefined>(undefined);
  backgroundActionError = stateValue('');
  backgroundActionErrorWorkspaceId = stateValue<string | undefined>(undefined);
  workspaceOrderMode = stateValue<WorkspaceOrderMode>('activity');
  manualWorkspaceOrder = stateValue<string[]>([]);
  workspacePreferencesError = stateValue('');
  activityOrder = stateValue<string[]>([]);
  activityRecords = stateValue<Map<string, WorkspaceActivityRecord>>(new Map());

  get displayedWorkspaces() {
    return sortWorkspaces(this.workspaces, this.workspaceOrderMode, this.manualWorkspaceOrder, this.activityOrder);
  }

  get shortcutWorkspaces() {
    return this.displayedWorkspaces.filter((workspace) => workspace.state === 'running');
  }

  get activeWorkspace() {
    return this.requestedWorkspaceId
      ? this.workspaces.find((workspace) => workspace.id === this.requestedWorkspaceId)
      : undefined;
  }

  get hasOpenWorkspace() {
    return Boolean(this.activeWorkspace || this.requestedWorkspaceId);
  }
  ...
  setWorkspaceOrderMode(mode: WorkspaceOrderMode) {
    ...
    if (mode === 'manual') {
      ...
      if (this.requestedWorkspaceId && this.#options.isWorkspaceObserved(this.requestedWorkspaceId)) {
        this.#activity.markWorkspaceObserved(this.requestedWorkspaceId);
        this.#activity.clearOutputActivity(this.requestedWorkspaceId);
      }
    }
  ...
  markWorkspaceObserved(workspaceId: string) {
    ...
    this.#activity.clearOutputActivity(workspaceId);
  }

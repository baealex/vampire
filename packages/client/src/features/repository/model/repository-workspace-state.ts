...
function stateValue<T>(): T | undefined;
function stateValue<T>(value: T): T;
function stateValue<T>(value?: T): T | undefined {
  return value;
}
...
export class RepositoryWorkspaceState {
  snapshot = stateValue<RepositorySnapshot>();
  loading = stateValue(true);
  errorMessage = stateValue('');
  refreshToken = stateValue(0);
  selection = stateValue<RepositorySelection>();
  openedFile = stateValue<WorkspaceFile>();
  fileDirty = stateValue(false);
  deleteTargets = stateValue<RepositoryDeleteTarget[]>([]);
  discardChangesPrompt = stateValue(false);
  uploading = stateValue(false);
  uploadNoticeKind = stateValue<RepositoryUploadNoticeKind>('');
  uploadNotice = stateValue('');
  uploadRevealRequest = stateValue<{ path: string; token: number }>();
  uploadConflicts = stateValue<RepositoryUploadConflict[]>([]);
  moving = stateValue(false);
  moveConflict = stateValue<RepositoryMoveConflict>();
  clipboard = stateValue<RepositoryClipboard>();
  discardTarget = stateValue<RepositoryChange>();
  discarding = stateValue(false);
  branchDeleteTarget = stateValue<RepositoryBranch>();
  deletingBranch = stateValue(false);
  loadingMoreCommits = stateValue(false);
  changeCount = stateValue(0);
  worktreeCount = stateValue(0);
  branch = stateValue<string>();

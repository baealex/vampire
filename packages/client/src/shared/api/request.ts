...
export async function requestResponse(path: string, init?: RequestInit, fallback = 'Request failed') {
  const response = await fetch(path, init);
  if (!response.ok) throw new RequestError(response.status, await readResponseError(response, fallback));
  return response;
}

export async function requestJson<T>(path: string, init?: RequestInit, fallback = 'Request failed'): Promise<T> {
  return (await requestResponse(path, init, fallback)).json() as Promise<T>;
}
